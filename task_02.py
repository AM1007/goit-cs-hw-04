...
# Initialize Colorama for working with colored text in the console
init(autoreset=True)
...
def threading_approach(file_paths, keywords):
    ...
    """
    threads = []
    results = ThreadQueue()  # Thread-safe queue to store results
    results_dict = {}
    ...
    def worker(file_paths_chunk, thread_id):
        ...
        for file_path in file_paths_chunk:
            search_keywords_in_file(file_path, keywords, local_results, f"Thread-{thread_id}")
...
def multiprocessing_worker(file_paths_chunk, keywords, queue, process_id):
    ...
    local_results = {}
    for file_path in file_paths_chunk:
        search_keywords_in_file(file_path, keywords, local_results, f"Process-{process_id}")
...
def multiprocessing_approach(file_paths, keywords):
    ...
    """
    processes = []
    queue = Queue()  # Queue for storing results
    results_dict = {}
    ...
    for i in range(num_processes):
        ...
        process = Process(target=multiprocessing_worker, args=(chunk, keywords, queue, i))
